...
public class JumpJumpHelper
{
    ...
    private static final String STORE_DIR               = "D:\\adb\\jump";
    ...
    private final String[]      ADB_SCREEN_CAPTURE_CMDS =
            { ADB_HOME+" shell screencap -p /sdcard/" + IMAGE_NAME,
                    ADB_HOME+" pull /sdcard/current.png " + STORE_DIR };
    ...
    private void doJump(double distance)
    {
        ...
        //执行按压操作
        String command = String.format(ADB_HOME+" shell input swipe %s %s %s %s %s", swipeX, swipeY, swipeX, swipeY,
                pressTime);
    ...
    {
        String command = String.format(ADB_HOME+" shell input tap %s %s", swipeX, swipeY);
        executeCommand(command);
    }
